...
export default function Home(){
 ...
 outer: for(let b=0;b<total;b+=batch){if(stopRef.current)break;const r=seeded(b+symbol.length*999+Date.now()%10000);for(let j=0;j<Math.min(batch,total-b);j++){
   ...
   if(live){
     ...
     if(tier.key==='15m'){
       const pool=sessionDayKeys.slice();const pickCount=Math.min(minTrades,pool.length)
   ...
   const passed=agg.trades>0&&agg.winRate>=45&&agg.returnPct>0&&agg.maxDrawdownPct<50
   const primary=sessions[0]
   const candidate={family,params:storedParams,result:{metrics:agg,trades:primary.trades},passed,reason:'',index,candles:primary.candles,startDate:primary.startDate,endDate:primary.endDate,sessions,testedAt:new Date().toISOString()} as Candidate
