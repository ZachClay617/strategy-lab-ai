...
type Strategy = { id:string; name:string; family:string; symbol:string; market:string; score:number; approved:boolean; metrics:any; explanation:string; parameters:any; equity:any; trades:Trade[]; candles?:Candle[]; test_start_at?:string|null; test_end_at?:string|null; created_at:string }
...
type Candidate = { family:string; params:any; result:any; passed:boolean; reason:string; index:number; candles:Candle[]; startDate?:string; endDate?:string }
...
function CandleChart({candles,trades,activeIndex,title,live=false,windowSize=50,errorMessage,indicator}:{candles:Candle[];trades:Trade[];activeIndex:number;title:string;live?:boolean;windowSize?:number;errorMessage?:string|null;indicator?:{label:string,color:string,values:(number|null)[]}|null}){
  ...
  return <div className="chart-wrap"><div className="chart-head"><div><b>{title}</b><span>{live?'LIVE MARKET DATA':'AI RESEARCH SIMULATION'}{indicator?` · ${indicator.label}`:''}</span></div><strong>{fmtPrice(candles[Math.min(activeIndex,candles.length-1)]?.close||0)}</strong></div><svg className="chart" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
...
export default function Home(){
 ...
 for(let b=0;b<total;b+=batch){if(stopRef.current)break;const r=seeded(b+symbol.length*999+Date.now()%10000);for(let j=0;j<Math.min(batch,total-b);j++){
   await waitWhilePaused();if(stopRef.current)break
   const index=completed+1;const family=families[Math.floor(r()*families.length)];const params=randomParams(r)
   const testCandles=live?data:randomWindow(data,r)
   ...
   const candidate={family,params,result,passed,reason:'',index,candles:testCandles,startDate:testCandles[0]?.date,endDate:testCandles[testCandles.length-1]?.date} as Candidate
   ...
   setActiveCandidate(candidate);setActiveTrades(result.trades);setActiveIndex(testCandles.length-1);setResearchCandles(testCandles);setBalance(result.endingBalance);setTestLog(prev=>[candidate,...prev].slice(0,18))
 ...
 await saveEvent(runId,summary,'success',100);setFeed(f=>[summary,...f]);setRunning(false);setPaused(false);setProgress(100);await loadData()
 }
 ...
 {selected&&<section className="panel replay"><div className="panel-title"><h2>STRATEGY REPLAY & WHY IT WORKED</h2><button className="ghost" onClick={()=>setSelected(null)}>CLOSE</button></div><h3>{selected.name}</h3><p className="muted">{selected.explanation}</p>{selected.test_start_at&&<p className="tiny">Tested {selected.symbol} from <b>{fmtDateTime(selected.test_start_at)}</b> to <b>{fmtDateTime(selected.test_end_at||undefined)}</b></p>}<div className="replay-grid"><div><span>Win rate</span><b>{selected.metrics.winRate.toFixed(1)}%</b></div><div><span>Return</span><b>{selected.metrics.returnPct.toFixed(2)}%</b></div><div><span>Drawdown</span><b>{selected.metrics.maxDrawdownPct.toFixed(1)}%</b></div><div><span>Sharpe</span><b>{selected.metrics.sharpe.toFixed(2)}</b></div><div><span>Trades</span><b>{selected.metrics.trades}</b></div></div>{selected.candles?.length&&selected.trades?.length?<CandleChart candles={selected.candles} trades={selected.trades} activeIndex={selected.candles.length-1} windowSize={Math.min(selected.candles.length,150)} title="Stored strategy trade map" indicator={indicatorSeries(selected.candles,selected.family,selected.parameters)}/>:<div className="chart empty">No stored chart for this strategy yet — run the migration and generate a new strategy to see its chart.</div>}<div className="section-label">EVERY TRADE THIS STRATEGY MADE</div><div className="trade-table">{selected.trades?.length?selected.trades.map((t,i)=><div className={`trade-row ${t.pnl>=0?'win':'loss'}`} key={i}><span>#{i+1}</span><span>{fmtDateTime(selected.candles?.[t.entryIndex]?.date)}</span><span>BUY {fmtPrice(t.entry)}</span><span>{fmtDateTime(selected.candles?.[t.exitIndex]?.date)}</span><span>SELL {fmtPrice(t.exit)}</span><span>{t.pnl>=0?'WIN':'LOSS'}</span><b>{fmtMoney(t.pnl)}</b></div>):<div className="empty">No trades recorded.</div>}</div></section>}
