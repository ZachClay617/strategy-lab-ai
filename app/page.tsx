...
function explainCandidate(c:Candidate,sessionsTested:number){const m=c.result.metrics;const n=sessionsTested||c.sessions?.length||1;const bar=tierLabel(c.sessions?.[0]?.tier);if(m.trades<=0)return`Rejected: no completed trades across ${n} tested session${n>1?'s':''} on ${bar} bars.`;if(m.winRate<45)return`Rejected: ${m.winRate.toFixed(1)}% win rate is below the required 45% on ${bar} bars. It finished with ${m.wins} wins and ${m.losses} losses.`;if(m.returnPct<=0)return`Rejected: the strategy reached the win-rate threshold but lost ${Math.abs(m.returnPct).toFixed(2)}% of starting capital on ${bar} bars.`;if(m.maxDrawdownPct>50)return`Rejected: drawdown reached ${m.maxDrawdownPct.toFixed(1)}%, so the risk profile was too deep on ${bar} bars.`;if(m.sharpe<.75)return`Rejected: ${m.sharpe.toFixed(2)} Sharpe is below the required 0.75 minimum on ${bar} bars.`;return`Qualified on ${bar} bars: ${m.wins} wins / ${m.losses} losses, ${m.returnPct.toFixed(2)}% return, ${m.maxDrawdownPct.toFixed(1)}% max drawdown and ${m.sharpe.toFixed(2)} Sharpe.`}
...
export default function Home(){
 ...
 outer: for(let b=0;b<total;b+=batch){if(stopRef.current)break;const r=seeded(b+symbol.length*999+Date.now()%10000);for(let j=0;j<Math.min(batch,total-b);j++){
   ...
   const passed=agg.trades>0&&agg.winRate>=45&&agg.returnPct>0&&agg.maxDrawdownPct<50&&agg.sharpe>=.75
