...
function categorizeRejection(m:any):string|null{
  ...
  if(m.returnPct<.07)return 'Return below 0.07%'
...
export default function Home(){
 ...
 outer: for(let b=0;b<total;b+=batch){if(stopRef.current)break;const r=seeded(b+symbol.length*999+Date.now()%10000);for(let j=0;j<Math.min(batch,total-b);j++){
   ...
   const passed=agg.trades>0&&agg.winRate>=45&&agg.returnPct>=.07&&agg.maxDrawdownPct<8&&agg.sharpe>=.85&&agg.sharpe<=1.5
   ...
   if(passed||index===1||index%uiStride===0){setActiveCandidate(candidate);setResearchCandles(primary.candles);setActiveTrades(primary.trades);setActiveIndex(primary.candles.length-1);setTestLog(prev=>[candidate,...prev].slice(0,200));setRejectionCounts({...rejections})}
 ...
 await saveEvent(runId,summary,'success',100);setFeed(f=>[summary,...f]);setRunning(false);setPaused(false);setProgress(100);setRejectionCounts({...rejections});await loadData();setBalance(capital)
 }
 async function resetBalance(){if(!supabase||!session?.user)return;await supabase.from('capital_events').insert({user_id:session.user.id,event_type:'reset',amount_before:balance,amount_after:STARTING_CAPITAL});const {error}=await supabase.from('profiles').upsert({id:session.user.id,current_balance:STARTING_CAPITAL,starting_balance:STARTING_CAPITAL});if(error){setMsg(`Could not save the reset: ${error.message}`);return}setBalance(STARTING_CAPITAL)}
 ...
 <section className="panel"><div className="panel-title"><h2>SUCCESSFUL STRATEGY LOG</h2><span className="muted">Saved to Supabase · grouped by run</span></div><div className="metrics"><div><span>Approval rule</span><b>≥45% WR + ≥0.07% return</b></div><div><span>Research capital</span><b>{fmtMoney(balance)}</b></div><div><span>Position mode</span><b>Long only, max 2% risk per trade</b></div><div><span>Market feed</span><b>Live chart</b></div></div>
