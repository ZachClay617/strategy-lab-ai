...
export default function Home(){
 ...
 outer: for(let b=0;b<total;b+=batch){if(stopRef.current)break;const r=seeded(b+symbol.length*999+Date.now()%10000);for(let j=0;j<Math.min(batch,total-b);j++){
   ...
   if(passed){qualified++;capital+=result.metrics.profit;setBalance(capital);best.push(candidate);best.sort((a,b)=>b.result.metrics.score-a.result.metrics.score);if(best.length>25)best.pop()}
   setActiveCandidate(candidate);setResearchCandles(testCandles);setActiveTrades(result.trades);setActiveIndex(testCandles.length-1)
 ...
 await supabase.from('profiles').update({current_balance:capital}).eq('id',session.user.id)
