...
export default function Home(){
 ...
 useEffect(()=>{
   if(!session?.user||market!=='Stocks'||symbol in symbolNames)return
   let dead=false
 ...
 async function loadData(){
   ...
   setMsg('');setStrategies((s||[]) as Strategy[]);setRuns((r||[]) as Run[]);const {data:p}=await supabase.from('profiles').select('current_balance').eq('id',session.user.id).maybeSingle();if(p?.current_balance!=null&&!runSnapshot.running)patchRun({balance:Number(p.current_balance)})
 }
 ...
 return <main className="shell"><header className="topbar"><div className="brand">◈ STRATEGY LAB <em>AI</em></div><div className="top-actions"><Link href="/account" className="pill account-pill">Account</Link><button className="ghost" onClick={signout}>Log out</button></div></header>
