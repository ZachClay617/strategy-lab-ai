...
export default function Home(){
 ...
 outer: for(let b=0;b<total;b+=batch){if(stopRef.current)break;const r=seeded(b+symbol.length*999+Date.now()%10000);for(let j=0;j<Math.min(batch,total-b);j++){
   ...
   const passed=agg.trades>0&&agg.winRate>=45&&agg.returnPct>=.25&&agg.maxDrawdownPct<50&&agg.sharpe>=.5&&agg.sharpe<=7
 ...
 <section className="panel"><div className="panel-title"><h2>SUCCESSFUL STRATEGY LOG</h2><span className="muted">Saved to Supabase · grouped by run</span></div><div className="metrics"><div><span>Approval rule</span><b>≥45% WR + ≥0.25% return</b></div><div><span>Research capital</span><b>{fmtMoney(balance)}</b></div><div><span>Position mode</span><b>Long only, max 2% risk per trade</b></div><div><span>Market feed</span><b>Live chart</b></div></div>
