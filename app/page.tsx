...
const STARTING_CAPITAL = 1_000_000_000_000_000_000
const REFILL_THRESHOLD = 50_000
const REFILL_AMOUNT = STARTING_CAPITAL
...
type Trade = { side:'LONG'|'SHORT'; entryIndex:number; exitIndex:number; entry:number; exit:number; qty:number; pnl:number; reason:string }
...
function backtest(data:Candle[], family:string, p:any){
  let cash=STARTING_CAPITAL, qty=0, side:'LONG'|'SHORT'|null=null, entry=0, entryIndex=0, wins=0, losses=0, trades=0, peak=cash, maxDD=0
  const equity:number[]=[]; const tradeLog:Trade[]=[]
  const riskFraction=.20
  const start=Math.max(35,p.slow,p.lookback)
  for(let i=start;i<data.length;i++){
    const c=data[i].close
    const {long,short}=signalFor(data,i,family,p)
    if(side===null && (long||short)){
      const notional=cash*riskFraction
      qty=notional/Math.max(c,.000001)
      side=long?'LONG':'SHORT';entry=c;entryIndex=i
    } else if(side==='LONG' && short){
      const pnl=(c-entry)*qty;cash+=pnl;const win=pnl>=0;if(win)wins++;else losses++;trades++;tradeLog.push({side,entryIndex,exitIndex:i,entry,exit:c,qty,pnl,reason:'Opposite signal closed the long position.'});side='SHORT';entry=c;entryIndex=i;qty=(cash*riskFraction)/Math.max(c,.000001)
    } else if(side==='SHORT' && long){
      const pnl=(entry-c)*qty;cash+=pnl;const win=pnl>=0;if(win)wins++;else losses++;trades++;tradeLog.push({side,entryIndex,exitIndex:i,entry,exit:c,qty,pnl,reason:'Opposite signal closed the short position.'});side='LONG';entry=c;entryIndex=i;qty=(cash*riskFraction)/Math.max(c,.000001)
    }
    const mark=side==='LONG'?cash+((c-entry)*qty):side==='SHORT'?cash+((entry-c)*qty):cash
    peak=Math.max(peak,mark);maxDD=Math.max(maxDD,(peak-mark)/Math.max(peak,1));equity.push(mark)
  }
  if(side){const c=data[data.length-1].close;const pnl=side==='LONG'?(c-entry)*qty:(entry-c)*qty;cash+=pnl;const win=pnl>=0;if(win)wins++;else losses++;trades++;tradeLog.push({side,entryIndex,exitIndex:data.length-1,entry,exit:c,qty,pnl,reason:'End-of-test liquidation.'})}
...
function CandleChart({candles,trades,activeIndex,title,live=false}:{candles:Candle[];trades:Trade[];activeIndex:number;title:string;live?:boolean}){
  const visible=candles.slice(Math.max(0,Math.min(candles.length-50,activeIndex-40)),Math.min(candles.length,Math.max(50,activeIndex+1)))
  ...
  return <div className="chart-wrap"><div className="chart-head"><div><b>{title}</b><span>{live?'LIVE MARKET DATA':'AI RESEARCH SIMULATION'}</span></div><strong>{fmtPrice(candles[Math.min(activeIndex,candles.length-1)]?.close||0)}</strong></div><svg className="chart" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
    ...
    {markers.map((t,i)=>{const ix=t.entryIndex-offset;const yy=y(t.entry);const exitIx=t.exitIndex-offset;const exitVisible=exitIx>=0&&exitIx<visible.length;return <g key={`${t.entryIndex}-${i}`}>{exitVisible&&<line x1={x(ix)} y1={yy} x2={x(exitIx)} y2={y(t.exit)} stroke={t.side==='LONG'?'#57e8ff':'#ff9b70'} strokeWidth="1.5" strokeDasharray="4 3" opacity=".8"/>}<circle cx={x(ix)} cy={yy} r="6" fill={t.side==='LONG'?'#57e8ff':'#ff9b70'} stroke="#0a1220" strokeWidth="1.5"/><text x={x(ix)+8} y={yy-8} fill={t.side==='LONG'?'#57e8ff':'#ff9b70'} fontSize="12" fontWeight="800">{t.side==='LONG'?'BUY':'SHORT'}</text></g>})}
  </svg></div>
}

export default function Home(){
 const [session,setSession]=useState<any>(null),[email,setEmail]=useState(''),[password,setPassword]=useState(''),[mode,setMode]=useState<'login'|'signup'>('login'),[msg,setMsg]=useState(''),[strategies,setStrategies]=useState<Strategy[]>([]),[runs,setRuns]=useState<Run[]>([]),[running,setRunning]=useState(false),[progress,setProgress]=useState(0),[feed,setFeed]=useState<string[]>([]),[testLog,setTestLog]=useState<Candidate[]>([]),[selected,setSelected]=useState<Strategy|null>(null),[symbol,setSymbol]=useState('AAPL'),[market,setMarket]=useState('Stocks'),[lookback,setLookback]=useState(1825),[variations,setVariations]=useState(25000),[minTrades,setMinTrades]=useState(10),[idea,setIdea]=useState(''),[sources,setSources]=useState({historical:true,paper:false,live:false}),[balance,setBalance]=useState(STARTING_CAPITAL),[researchCandles,setResearchCandles]=useState<Candle[]>([]),[activeIndex,setActiveIndex]=useState(0),[activeTrades,setActiveTrades]=useState<Trade[]>([]),[activeCandidate,setActiveCandidate]=useState<Candidate|null>(null),[liveCandles,setLiveCandles]=useState<Candle[]>([]),[liveStatus,setLiveStatus]=useState('Waiting for live market data'),[selectedRun,setSelectedRun]=useState<Run|null>(null),[speed,setSpeed]=useState<typeof speeds[number]['key']>('normal'),[usingLiveData,setUsingLiveData]=useState(false)
 ...
 useEffect(()=>{if(!session?.user||market!=='Stocks')return;let dead=false;const load=async()=>{try{const r=await fetch(`/api/market?symbol=${encodeURIComponent(symbol)}&live=1`);const j=await r.json();if(!dead&&Array.isArray(j)&&j.length){setLiveCandles(j);setLiveStatus(`Updated ${new Date().toLocaleTimeString()}`)}}catch{if(!dead)setLiveStatus('Live feed unavailable')}};load();const id=setInterval(load,15000);return()=>{dead=true;clearInterval(id)}},[session?.user?.id,symbol,market])
 ...
 async function runResearch(){if(!supabase||!session?.user||running)return;stopRef.current=false;setRunning(true);setProgress(0);setFeed([]);setTestLog([]);setSelectedRun(null);setActiveCandidate(null);const runId=crypto.randomUUID();const started=new Date().toISOString();const {error:runInsertError}=await supabase.from('research_runs').insert({id:runId,user_id:session.user.id,symbol:symbol.toUpperCase(),market,modes:Object.entries(sources).filter(([,v])=>v).map(([k])=>k),variations_requested:variations,status:'running',started_at:started,starting_balance:STARTING_CAPITAL,current_balance:STARTING_CAPITAL,tested_count:0,qualified_count:0,summary:'AI research started.',capital_events:[]});
 if(runInsertError){setMsg(`Could not start the run: ${runInsertError.message}`);setRunning(false);return}
 setMsg('')
 let data:Candle[]=[];const live=sources.live
 if(live){
   try{const res=await fetch(`/api/market?symbol=${encodeURIComponent(symbol)}&live=1&market=${encodeURIComponent(market)}`);const j=await res.json();if(!Array.isArray(j)||j.length<40)throw new Error('insufficient live data');data=j;setLiveCandles(j);setLiveStatus(`Updated ${new Date().toLocaleTimeString()}`);setUsingLiveData(true);setFeed(f=>['Live execution: testing against real-time market data instead of historical bars.',...f])}
   ...
   try{const res=await fetch(`/api/market?symbol=${encodeURIComponent(symbol)}&days=${lookback}&market=${encodeURIComponent(market)}`);data=await res.json();if(!Array.isArray(data)||data.length<60)throw new Error('insufficient data')}catch{data=makeSynthetic(symbol,lookback);setFeed(f=>['Market adapter unavailable; using clearly labeled development data.',...f])}
 }
 ...
 for(const item of best.slice(0,25)){const m=item.result.metrics;const {error}=await supabase.from('strategies').insert({user_id:session.user.id,run_id:runId,symbol:symbol.toUpperCase(),market,name:`${item.family} / ${Math.round(m.score)} score`,family:item.family,parameters:item.params,source:idea?'AI + user idea':'AI generated',approved:true,score:m.score,metrics:m,equity:item.result.equity,trades:item.result.trades,explanation:`Why it worked: ${item.family} generated ${m.wins} wins and ${m.losses} losses for a ${m.winRate.toFixed(1)}% win rate. It returned ${m.returnPct.toFixed(2)}% with ${m.maxDrawdownPct.toFixed(1)}% maximum drawdown and ${m.sharpe.toFixed(2)} Sharpe. The engine used both long and short positions and closed positions on opposing signals or at the end of the test. This is historical research, not a guarantee of future performance.`});if(error)console.error('strategy save failed',error)}
 ...
 <section className="hero"><div><div className="eyebrow">AI STRATEGY RESEARCH ENGINE</div><h1>Build. Break. <span>Repeat.</span></h1><p className="muted">Watch the engine generate, test, trade long and short, reject weak ideas, and save the strategies that pass the rules.</p></div><div className="balance"><small>AI RESEARCH CAPITAL</small><strong>{fmtMoney(balance)}</strong><button onClick={resetBalance}>RESET TO $1 QUINTILLION</button></div></section>
 <div className="grid"><section className="panel"><div className="panel-title"><h2>RUN CONFIGURATION</h2><span className="badge">45% MIN WIN RATE</span></div><label>Symbol<input value={symbol} onChange={e=>setSymbol(e.target.value.toUpperCase())}/></label><label>Market<select value={market} onChange={e=>setMarket(e.target.value)}>{markets.map(x=><option key={x}>{x}</option>)}</select></label><label>Lookback<select value={lookback} onChange={e=>setLookback(+e.target.value)}><option value={365}>1 year</option><option value={1095}>3 years</option><option value={1825}>5 years</option><option value={3650}>10 years</option></select></label><label>Strategy variations (up to {MAX_VARIATIONS.toLocaleString()})<input type="number" min={50} max={MAX_VARIATIONS} value={variations} onChange={e=>setVariations(clamp(Math.round(+e.target.value||0),50,MAX_VARIATIONS))}/></label><label>Minimum trades (up to {MAX_MIN_TRADES.toLocaleString()})<input type="number" min={1} max={MAX_MIN_TRADES} value={minTrades} onChange={e=>setMinTrades(clamp(Math.round(+e.target.value||0),1,MAX_MIN_TRADES))}/></label><label>Test speed<select value={speed} onChange={e=>setSpeed(e.target.value as typeof speed)}>{speeds.map(s=><option key={s.key} value={s.key}>{s.label}</option>)}</select></label><label>User strategy idea<textarea value={idea} onChange={e=>setIdea(e.target.value)} placeholder="Optional: describe a strategy you want tested..."/></label><div className="section-label">TEST SOURCES</div><div className="checks">{(['historical','paper','live'] as const).map(k=><label key={k}><input type="checkbox" checked={sources[k]} onChange={e=>setSources({...sources,[k]:e.target.checked})}/>{k==='live'?'Live execution':k[0].toUpperCase()+k.slice(1)}</label>)}</div><button className="run" onClick={runResearch} disabled={running}>{running?`AI TRADING · ${progress.toFixed(1)}%`:'▶ RUN AI SEARCH'}</button><p className="tiny">Live execution runs the AI's tests against live, real-time market data instead of historical bars. It never places a real trade or touches a broker account.</p></section>
 <section className="panel"><div className="panel-title"><h2>LIVE MARKET CHART</h2><span className="muted">{running&&usingLiveData?'AI TRADING ON LIVE DATA':liveStatus}</span></div>{running&&usingLiveData&&researchCandles.length?<CandleChart candles={researchCandles} trades={activeTrades} activeIndex={activeIndex} title={activeCandidate?`TEST #${activeCandidate.index.toLocaleString()} · ${activeCandidate.family}`:`${symbol} · live feed`} live/>:liveCandles.length?<CandleChart candles={liveCandles} trades={[]} activeIndex={liveCandles.length-1} title={`${symbol} · live feed`} live/>:<div className="chart empty">Waiting for live stock candles…</div>}<p className="tiny">This is market-data visualization only. Data availability and delay depend on the public market-data feed.</p></section></div>
 <div className="grid"><section className="panel"><div className="panel-title"><h2>AI TRADING ARENA</h2><span className={running?'live-dot':''}>{running?'ENGINE TRADING':'READY'}</span></div>{researchCandles.length?<CandleChart candles={researchCandles} trades={activeTrades} activeIndex={activeIndex} title={activeCandidate?`TEST #${activeCandidate.index.toLocaleString()} · ${activeCandidate.family}`:'Research chart'}/>:<div className="chart empty">Run a search to watch long/short trades appear on the candlestick chart.</div>}<div className="trade-status"><div><span>Current test</span><b>{activeCandidate?`${activeCandidate.family} · ${activeCandidate.passed?'QUALIFIED':'REJECTED'}`:'—'}</b></div><div><span>Outcome</span><b>{activeCandidate?.reason||'Waiting for the first strategy test.'}</b></div></div></section>
 ...
 <section className="panel"><div className="panel-title"><h2>SUCCESSFUL STRATEGY LOG</h2><span className="muted">Saved to Supabase · sorted by score</span></div><div className="metrics"><div><span>Approval rule</span><b>≥45% WR + positive return</b></div><div><span>Research capital</span><b>$1 QUINTILLION</b></div><div><span>Position mode</span><b>Long + Short</b></div><div><span>Market feed</span><b>Live chart</b></div></div><div className="table">{strategies.length===0?<div className="empty">No successful strategies saved yet. Run the engine and qualified strategies will appear here.</div>:strategies.map(s=><button className="row" key={s.id} onClick={()=>setSelected(s)}><div><strong>{s.name}</strong><span>{s.symbol} · {s.market} · {s.family}</span></div><b>{s.score.toFixed(1)}</b><span>{s.metrics?.winRate?.toFixed(1)}% WR</span><span>{s.metrics?.returnPct?.toFixed(1)}% return</span><span>{s.metrics?.maxDrawdownPct?.toFixed(1)}% DD</span><span>{s.metrics?.trades} trades</span></button>)}</div></section>
